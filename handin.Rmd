...
```
The cross-validation results for the random forest model demonstrate a RMSE of 0.193 with a standard error of 0.007 and an R-squared of 0.601, indicating strong predictive performance.
...
```
The Random Forest model has a significantly lower RMSE and higher RSQ compared to the Linear Regression model, suggesting that it provides a more accurate prediction. \\
...
mtry_range <- tree_res$.metrics[[1]] %>%
  select(mtry) %>%
  summarise(min_mtry = min(mtry),
            max_mtry = max(mtry))

min_n_range <- tree_res$.metrics[[1]] %>%
  select(min_n) %>%
  summarise(min_min_n = min(min_n),
            max_min_n = max(min_n))
